...
export function createSingleEliminationBracket(participantIds, mode = 'team') {
  ...
  if (prelimCount > 0) {
    ...
    for (let i = 0; i < prelimCount; i++) {
      ...
      bracket.r1.push(makeMatch(`r1_m${i + 1}`, a.id, b.id, mode, round, false));
  ...
  while (nextPool.length > 1) {
    ...
    for (let i = 0; i < nextPool.length; i += 2) {
      ...
      const match = makeMatch(matchId, a.type === 'seed' ? a.id : null, b.type === 'seed' ? b.id : null, mode, round, isFinal, isSemiFinal);
    ...
    if (isSemiFinal) {
      bracket.bronze = [makeMatch('bronze', null, null, mode, round + 1, false, false, true)];
...
function makeMatch(id, teamA, teamB, mode, round, isFinal = false, isSemiFinal = false, isBronze = false) {
  const format = mode === 'brawl' ? 1 : (isFinal ? 5 : isSemiFinal || isBronze ? 3 : 1);
  return { id, teamA, teamB, scoreA: 0, scoreB: 0, format, completed: false, winner: null };
}
